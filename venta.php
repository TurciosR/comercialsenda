...
function initial()
{
    ...
    $id_usuario = $id_user;
    $fecha_actual = date('Y-m-d'); //array clientes //factura
    //clientes
    if (isset($_REQUEST['id_pedido'])) {
        $id_pedido = $_REQUEST['id_pedido'];
        $sql_pedido = _query("SELECT co.*, cl.nombre
                            ...
                            if ($turno_vigente == '1') { ?>
                                <div class="ibox-content">
                                    ...
                                    <div class="row focuss"><br>
                                        <!-- BUSQUEDA DE PRODUCTO (CODIGO O DESCRIPCION) -->
                                        ...
                                        <!-- BOTONES MULTIFUNCION DE LA SECCION DE VENTAS -->
                                        <div class="col-md-7"><br>
                                            ...
                                            <button type="button" id="preventa" style="margin-left:1%;" name="preventa" class="btn btn-sm btn-success pull-right usage"><i class="fa fa-save"></i> F8
                                                Guardar</button>
                                            <button type="button" id="borrar_preven" style="margin-left:1%;" name="borrar_preven" class="btn btn-sm btn-success pull-right usage"><i class="fa fa-trash"></i> F6
                                                Borrar </button>

                                            <?php if (
                                                isset($_REQUEST['id_pedido'])
                                            ) {
                                    ...
                                    <div class="row">
                                        ...
                                        <!-- INPUT DE AUTORIZACION (COMENTADO POR EL MOMENTO) -->
                                        <!-- <div class="form-group col-md-1">
                                            <div class="form-group has-info">
                                                <label>Autorización</label>
                                                <input maxlength="6" type="text" class="form-control" id="aut_code" name="aut_code" value="">
                                            </div>
                                        </div> -->
                                    </div>
                                                                                            ...
                                                                                            if (
                                                                                                ...
                                                                                                if (
                                                                                                    ...
                                                                                                    $categoria =
                                                                                                        $row1[
                                                                                                            'id_categoria'
                                                                                                        ]; // $sql_res_pre=_fetch_array(_query("SELECT SUM(factura_detalle.cantidad) as reserva FROM factura JOIN factura_detalle ON factura_detalle.id_factura=factura.id_factura WHERE factura_detalle.id_prod_serv=$id_producto AND factura.id_sucursal=$id_sucursal AND factura.fecha = '$hoy' AND factura.finalizada=0 ")); // $reserva=$sql_res_pre['reserva']; // $sql_res_esto=_fetch_array(_query("SELECT SUM(factura_detalle.cantidad) as reservado FROM factura JOIN factura_detalle ON factura_detalle.id_factura=factura.id_factura WHERE factura_detalle.id_prod_serv=$id_producto AND factura.id_factura=$id_factura")); // $reservado=$sql_res_esto['reservado']; // $stock= $row1["stock"]-$reserva+$reservado;
                                                                                                    //
                                                                                                    //
                                                                                                    //
                                                                                                    // if ($stock<0) {
  ...
  AND su.id_sucursal=$id_sucursal";
    ...
    if ($nrow1 > 0) {
        if ($row1['descripcion'] != '' && $row1['descripcion'] != null) {
            ...
            $sql_exis = _query(
                "SELECT stock FROM stock WHERE id_producto = '$id_productov' AND id_sucursal=" .
                    $id_sucursal
            );
...
function insertar()
{
    ...
    $tipo_entrada_salida = '';
    $rows = _fetch_array(
        _query("SELECT * FROM correlativo WHERE id_sucursal=$id_sucursal")
    );
    $ult_ccf = $rows['ccf'] + 1;
    $ult_cof = $rows['cof'] + 1;
    ...
    if ($tipo_impresion == 'COF') {
        ...
        $tipo_entrada_salida = 'FACTURA CONSUMIDOR';
        $data_numdoc = ['nota_envio' => $ult_nota_envio];
    ...
    if ($tipo_impresion == 'TIK') {
        ...
        $serie = $swl['serie_cof'];
        $sql_ult = _query("SELECT MAX(CONVERT(num_fact_impresa,UNSIGNED INTEGER))as ultimo
      FROM factura
      WHERE id_sucursal=$id_sucursal
    ...
    if ($cuantos > 0) {
        ...
        foreach ($array as $fila) {
            ...
            $sql_costo = _fetch_array(
                _query("SELECT costo FROM presentacion_producto WHERE id_pp  = $id_presentacion")
            );
        ...
        ORDER BY id_posicion DESC ,id_estante DESC"
            ...
            ORDER BY vencimiento");
                ...
                /*Hay suficientes unidades en  el stock_ubicacion para realizar el descargo y se procede normalmente*/ $table1 =
                    'movimiento_producto_detalle';
                $cant_total = $existencias - $cantidad;
                $form_data1 = [
                    'id_movimiento' => $id_movimiento,
...
function imprimir_fact()
{
    ...
    if ($tipo_impresion == 'ENV') {
        /**
         * FALTA APLICAR PASITO PROHIBIDO BY MOU
         */
