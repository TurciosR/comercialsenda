<?php
    ...
    $cat_row = _fetch_array(_query("SELECT * from categoria where id_categoria=$id_categoria"));
    $categoria = $cat_row['nombre_cat'];
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A3', $title0." ".mb_strtoupper($categoria));
    ...
    $sql_stock = _query("SELECT pr.id_producto,pr.descripcion, pr.barcode, c.nombre_cat as cat, SUM(su.stock) as cantidad
                         ...
                         WHERE su.stock>0 AND su.id_sucursal='$id_sucursal' AND pr.id_categoria=$id_categoria GROUP BY su.id_producto ORDER BY pr.descripcion");
