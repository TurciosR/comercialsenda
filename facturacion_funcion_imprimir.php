...
function print_fact($id_factura,$tipo_id,$nombreapecte,$direccion){
	...
	if($nrows_fact>0){
		...
		$arrayL[4]=  p_set($arrayL[4],$diaa,11,16,"B");
		$arrayL[4]=  p_set($arrayL[4],$mess,26,35,"B");
		$arrayL[4]=  p_set($arrayL[4],$anio,46,51,"B");

		$arrayL[7]= p_set($arrayL[7],$nombre_ape,16,95,"R");

		$arrayL[9]= p_set($arrayL[9],$direccionx,16,95,"R");


		if($nodp=="0000-000000-000-0"){$nodp="";}
		$arrayL[11]= p_set($arrayL[11],$nodp,15,42,"R");
		...
		for($i=0;$i<$nrows_fact_det;$i++){
			...
			if ($i<29) {
				...
				if ($exento==0){
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$cantidad,3,8,"B");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),12,55,"R");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$precio_unit,59,64,"L");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$subtotal,86,95,"L");
				}
				else{
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$cantidad,3,8,"B");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),12,55,"R");
	...
	$arrayL[46]= p_set($arrayL[46],sprintf("%.4f",$SumGravado),86,95,"L");
	$arrayL[48]= p_set($arrayL[48],sprintf("%.4f",$row_fact['retencion']),86,95,"L");
	$arrayL[50]= p_set($arrayL[50],sprintf("%.4f",round(($SumGravado-$row_fact['retencion']),4)),86,95,"L");
	$arrayL[52]= p_set($arrayL[52],sprintf("%.4f","0.0000"),86,95,"L");
	$arrayL[54]= p_set($arrayL[54],sprintf("%.4f",$venta_exenta),86,95,"L");
	$arrayL[56]= p_set($arrayL[56],sprintf("%.2f",round(($SumGravado+$venta_exenta-$row_fact['retencion']),2)),86,95,"L");
...
function print_ccf($id_fact,$tipo_id,$nitcte,$nrccte,$nombreapecte,$direccion){
	...
	if($nrows_fact>0){
		...
		$depart="";
		$municipio_cliente="";
		//CONSULTA PARA TRAER EL DEPARTAMENTO y el minicipio DEL CLIENTE
		$sql="select cliente.*, departamento.nombre_departamento, municipio.nombre_municipio from cliente
			LEFT JOIN departamento ON departamento.id_departamento=cliente.depto
			LEFT JOIN municipio ON municipio.id_municipio=cliente.municipio where id_cliente='$id_cliente'";
		$result=_query($sql);
		...
		if ($count > 0) {
				for ($i = 0; $i < $count; $i ++) {
						...
						$municipio_cliente=$row1['nombre_municipio'];
				}
		}

		$nombre_ape=$nombres;
		$dir_txt= quitar_spc($direccion);
		$giro_cte1=$giro_cte;
		$total_final=0;
		...
		$arrayL[5]=  p_set($arrayL[5],$diaa,43,49,"B");
		$arrayL[5]=  p_set($arrayL[5],$mess,56,79,"B");
		$arrayL[5]=  p_set($arrayL[5],$anio,83,93,"B");


		$arrayL[7]=  p_set($arrayL[7],$nombreapecte,13,93,"R");

		$arrayL[9]=  p_set($arrayL[9],$dir_txt,15,93,"R");
		$arrayL[11]=  p_set($arrayL[11],$municipio_cliente,17,49,"R");
		$arrayL[11]=  p_set($arrayL[11],$nrccte,62,93,"R");		
		$arrayL[13]=  p_set($arrayL[13],$depart,21,49,"R");

		$arrayL[12]=  p_set($arrayL[12],substr($giro_cte1,0,30),62,93,"R");
		$arrayL[13]=  p_set($arrayL[13],substr($giro_cte1,30),62,93,"R");


		$arrayL[15]=  p_set($arrayL[15],$nitcte,63,93,"R");
		...
		for($i=0;$i<$nrows_fact_det;$i++){
			...
			if ($i<20) {
				// code...
				if ($exento==0){
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$cantidad,3,9,"B");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),14,55,"R");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$precio_sin_iva,61,66,"L");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],number_format(($subtotal/1.13),4,".",""),82,93,"L");
				}
				if ($exento==1){
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$cantidad,3,9,"B");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),14,55,"R");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$precio_sin_iva,61,66,"L");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$subtotal,82,93,"L");
					}
			}
			$cuantos=$cuantos+1;
		}
	...
	$array_nocon= dtl($cadena_salida_txt,42);
	foreach ($array_nocon as $key => $value) {
		// code...

		$arrayL[$array_painc[$key]]=p_set($arrayL[$array_painc[$key]],$value,11,55,"B");
	}
	$arrayL[46] = p_set($arrayL[46],$total_value_gravado,83,93,"L");
	//$arrayL[45] = p_set($arrayL[45],"0.0000",87,99,"L");
	$arrayL[47] = p_set($arrayL[47],$total_iva_format,83,93,"L");
	$arrayL[49] = p_set($arrayL[49],$subtotal_gravado_print,83,93,"L");
	//$arrayL[51] = p_set($arrayL[51],$retencion,83,93,"L");
	$arrayL[55] = p_set($arrayL[55],$total_value_exento,83,93,"L");
	$arrayL[57] = p_set($arrayL[57],sprintf("%.2f",$total_final_format),83,93,"L");
...
function print_corte($id_corte){
	...
	if($tipo=="C"){
		...
		$sql_pagos = _query("SELECT * FROM factura where id_apertura_pagada ='$id_apertura' AND finalizada=1 AND anulada = 0 and credito in (2,3)");
		$cuenta_pagos = _num_rows($sql_pagos);
		$total_pagos = 0;
		if($cuenta_pagos > 0)
		{

			$info_factura.=str_pad("\nCARGOS PAGO TARJETA",36," ",STR_PAD_BOTH)."\n\n";
			while ($row_cuenta = _fetch_array($sql_pagos))
			{
				$monto = $row_cuenta["cargo_tarjeta"];
				$nombre_text = $row_cuenta['tipo_documento']." ".$row_cuenta['num_fact_impresa'];
				$info_factura.=
				 str_pad($nombre_text,21," ",STR_PAD_RIGHT)
				.str_pad(number_format($monto,2),15," ",STR_PAD_LEFT)
				."\n";
				$total_pagos += $monto;
			}
			$info_factura.=
			 str_pad("TOTAL",21," ",STR_PAD_RIGHT)
			.str_pad(number_format($total_pagos,2),15," ",STR_PAD_LEFT)
			."\n";
		}
	}
...
function quitar_spc($cadena){
	$no_permitidas= array ("ª","º","á","é","í","ó","ú","Á","É","Í","Ó","Ú","À","Ã","Ì","Ò","Ù","Ã™","Ã ","Ã¨","Ã¬","Ã²","Ã¹","ç","Ç","Ã¢","ê","Ã®","Ã´","Ã»","Ã‚","ÃŠ","ÃŽ","Ã”","Ã›","ü","Ã¶","Ã–","Ã¯","Ã¤","«","Ò","Ã","Ã„","Ã‹");
