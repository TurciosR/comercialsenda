    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.typeinfo == "Success") {
                ...
                var preciop = parseFloat(data.preciop);
                ...
                /*if (tipo_doc == 'CCF')
                {
                  tr_add += "<td class='col-lg-1'><div>" + unit + "<input type='text' id='costoProd'  class='form-control precio_compra' value='" + round((cp/1.13),4) + "' data-cporiginal='"+ cp +"' style='width:80px;'></div></td>";
                }
                else {
                  tr_add += "<td class='col-lg-1'><div>" + unit + "<input type='text'  class='form-control precio_compra' value='" + cp + "' style='width:80px;'></div></td>";
                }*/
                tr_add += "<td class='col-lg-1'><div>" + unit + "<input type='text' id='costoProd'  class='form-control precio_compra' value='" + cp + "' data-cporiginal='"+ cp +"' style='width:80px;'></div></td>";
...
function totales(proc = "prec") {
    ...
    $("#inventable tr").each(function() {      
        let cporiginal = $(this).find("#costoProd").data('cporiginal');
        cporiginal = parseFloat(cporiginal);
        let preciomod="";
        if(tipo_doc =='CCF'){
          preciomod = round((cporiginal/1.13),4);
        }
        else {
          preciomod = cporiginal;
        }
        $(this).find(".precio_compra").val(preciomod);
...
$(document).on('change', '.sel', function(event) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $(a).find("#costoProd").data('cporiginal', data.costo);
            totales();
        }
    });
});
